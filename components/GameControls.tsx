    ...
    return (
        <div className="w-full p-3 bg-[#fff1e6]/60 rounded-lg flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div className="flex gap-2">
                {isPlayerTurn ? (
                    <>
                        <button onClick={onDrawTile} disabled={canPlayerPlay || heapCount === 0 || drawCountThisTurn >= 3} className="px-5 py-2 bg-[#83c5be] text-white rounded-lg font-semibold hover:bg-[#83c5be]/80 disabled:bg-gray-400 disabled:cursor-not-allowed">{t('drawTileButton')} ({3 - drawCountThisTurn})</button>
                        <button onClick={onPassTurn} disabled={canPlayerPlay} className="px-5 py-2 bg-[#fff1e6] text-[#2f3e46] rounded-lg font-semibold hover:bg-[#fff1e6]/80 disabled:bg-gray-400 disabled:cursor-not-allowed">{t('passTurnButton')}</button>
                    </>
